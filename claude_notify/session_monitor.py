...
class ClaudeSessionMonitor:
    ...
    def _read_transcript_tail(
        self, transcript_path: Path, lines: int = 50
    ) -> List[str]:
        """Read the last N lines of a transcript file.

        The file is streamed through a bounded ``deque`` so memory stays
        O(lines) regardless of transcript size, instead of loading the whole
        file just to slice off the tail.
        """
        try:
    ...
    def _analyze_transcript_state(self, transcript_lines: List[str]) -> Dict[str, Any]:
        ...
        # Join lines for analysis (lower-cased once, reused by every match below)
        ...
        # Check if the last non-empty line is from Claude (heuristic). Scan from
        # the end so we don't materialize the whole transcript just for the tail.
        last_line = next(
            (line.strip() for line in reversed(transcript_lines) if line.strip()),
            "",
        )
        # A trailing question mark suggests Claude is waiting on an answer.
    ...
    def check_sessions(self) -> List[Dict[str, Any]]:
        ...
        for transcript_path, project_dir in active_sessions:
            ...
            last_mtime = self.transcript_states.get(
                str(transcript_path), {}
            ).get("mtime", 0)
