...
class ClaudeNotifier:
    ...
    def _send_windows_notification(
        self, title: str, message: str, timeout: int
    ) -> bool:
        ...
        ps_script = (
            ...
            "$title = [System.Security.SecurityElement]"
            "::Escape($env:CLAUDE_NOTIFY_TITLE)\n"
            "$message = [System.Security.SecurityElement]"
            "::Escape($env:CLAUDE_NOTIFY_MESSAGE)\n"
    ...
    def check_dependencies(self) -> Dict[str, Any]:
        ...
        The mapping is heterogeneous: ``plyer``/``native`` are booleans while
        ``method`` (added per platform below) is the backend name, hence the
